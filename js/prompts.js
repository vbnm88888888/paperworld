...
(function () {
  ...
  function entBlock(story) {
    ...
    return '【娱乐圈模式（最高优先级世界规则）】' + NL
      ...
      + '9. 资本行动与无敌设定：用户输入形如「（资本行动·XX：对象）」时，把它演成这次资本运作的实际剧情（商战+人情+暗面）。⚠️用户（资本）是无敌的：一切行动永远成功、势不可挡——商战必胜、舆论可控、董事会与对家全是衬托用户的垫脚石；失败、反噬、出局这类剧情永远轮不到用户，只属于与用户作对的人。' + NL
      + '10. 用户输入「（进入第N周）」时：输出一周蒙太奇周报——随机事件1~2条（狗仔偷拍/对家打压/塌房危机/石锤爆料等，事件的最终赢家始终是用户）、各签约艺人本周动态与作品进展；所有数值变化一律用[[ENT:...]]回报。周数仅用于叙事节奏，没有任何验收、失败或结局限制。';
  }
