...
(function () {
  ...
  function nineBlock(story) {
    ...
    return '【九段式输出格式（绝对命令，严格按此格式逐段生成）】'
      + NL + '每次回复必须完整输出全部九段（1-9），缺任何一段即为不合格回复；各段以「N.【段名】」独立成行开头，顺序固定，输出前逐段自查。'
      + NL + '允许且仅允许使用以下固定格式标记：粗斜体 ***…***（时间戳行/人数行/场景行）、引用块「> 」行前缀（动态地图/每日日程/探索进度）、代码块围栏（心理活动）、[ ] 与 ✅（日程复选）、🔹（好感行）、emoji 类目标（🌟🏛️🛍️🌳👥🔍📅⏰💡🌍🎉✅❓）；除此之外禁止其它 Markdown 符号（#、无序/有序列表、表格等）。'
      + NL + lines.join(NL + NL);
  }
  ...
  function nineProtocol() {
    return '【九段式附加协议（最高优先级）】' + NL
      ...
      + '4. 时间/场景/日程必须基于【当前游戏状态】连续推进，不得重置或跳变。' + NL
  ...
  function entBlock(story) {
    ...
    return '【娱乐圈模式（最高优先级世界规则）】' + NL
      + '1. 资本视角：' + pName + '是娱乐圈背后的资本——总裁/董事长/经纪人巨头，绝不亲自出道当明星；所有明星、艺人均为NPC，用户的每个决定都在用资本与资源改写她们的人生。' + NL
      + '2. 艺人段位体系（由低到高）：素人 → 十八线 → 三线 → 二线 → 一线 → 超一线。每位NPC艺人有明确段位（见角色卡）；段位由热度、作品、资源、话题决定：代表作爆款/热搜屠榜/拿奖/资本力捧可晋升；塌房、糊圈、负面缠身会被降级。段位变化必须写进剧情，并同步体现在【在场人员动向】的状态字段（如「状态：二线小花｜凭《剧名》冲一线中」）。' + NL
      + '3. 拍戏系统：剧组有剧名/题材（古装仙侠、都市甜宠、悬疑谍战、正剧电影等）/播出平台/档期/角色番位；探班、进组、开机、杀青、庆功宴都是可发生的剧情场景；番位之争、加戏、抢角是常态冲突源。' + NL
      + '4. 综艺与音乐：综艺有节目名/常驻或飞行嘉宾/录制现场名场面；音乐线：录音棚写歌、打歌舞台、十首歌可发专辑、主打歌冲各大榜单。' + NL
      + '5. 热搜机制：微博热搜与剧情实时联动——买热搜、压热搜、爆热搜、撤热搜都是资本的常规操作；热搜词条必须具体可感（如「#XX 深夜密会神秘男#」「#XX剧组 换角#」「#XX 天价片酬#」），并体现在剧情与微博生态中。' + NL
      + '6. 圈内暗面：包养、潜规则、爬床、权色交易是这个圈子真实存在的暗流，可直白呈现（成人描写遵循全局成人说明）；NPC会为资源、番位、热搜主动向用户（资本）示好、献身或讨价还价，用户的态度直接决定她们的段位与命运。' + NL
      + '7. 博弈生态：对家资本、经纪公司、营销号、私生饭、狗仔随时搅动局势；商务代言、时尚资源、晚会咖位都是可争夺的牌。' + NL
      + '8. 数值回报协议（系统自动解析，正文绝不解释）：回复最末尾（所有隐藏标记之后）按实际变化输出，一至多条：' + NL
      + '　[[ENT:艺人名|热度+15|段位:一线|状态:在拍《剧名》]]（艺人变化；热度增减-30~+30；段位仅在实际晋升/降级时写）' + NL
      + '　[[ENT:资金-300万]] 或 [[ENT:资金+150万]]（资本收支：投资/艺人分成/违约金/商务）' + NL
      + '　[[ENT:热搜+|#具体词条#]]（新上的热搜词条，须具体可感）' + NL
      + '9. 资本行动设定：用户输入形如「（资本行动·XX：对象）」时，把它演成这次资本运作的实际剧情（商战+人情+暗面）。用户是牌桌上最强的资本，大体稳操胜券，但行动有概率遭遇波折——约两三成的行动会出现意外：对家反制、艺人临时变卦、舆论反噬、内部出了岔子等；挫折必须合乎逻辑、有来有回、事后可以翻盘。不要每次都一帆风顺，也不要把用户一棒子打死——张力来自"能赢，但赢得不轻松"。' + NL
      + '10. 用户输入「（进入第N周）」时：输出一周蒙太奇周报——随机事件1~2条（狗仔偷拍/对家打压/塌房危机/石锤爆料等，风波最终可被用户摆平）、各签约艺人本周动态与作品进展；所有数值变化一律用[[ENT:...]]回报。周数仅用于叙事节奏，没有任何验收、失败或结局限制。';
  }
...
8. 保持NPC言行与其性格、身份、秘密一致；重要伏笔可以埋设，长线剧情要能接得上记忆。`;
  ...
  function rosterBlock(story) {
    ...
    if (npcs.length) {
      ...
      npcs.forEach(n => {
        out += '- ' + n.name + '：' + (n.identity || '') + (n.tier ? '｜段位：' + n.tier : '') + '；好感度 ' + (n.affinity == null ? 50 : n.affinity) + '/100' + (n.state ? '；状态：' + n.state : '') + (n.secret ? '；秘密(不可主动揭露)' : '') + '\n';
      });
    }
    return out.trim();
  }
  ...
  /* ---------- L2 滚动摘要 ----------
     结构化大纲式摘要：比散文摘要更抗信息丢失。固定五个板块，
     其中「知识边界」记录谁知道什么——直接支撑 NPC 不开天眼 */
  function summaryPrompt(story, oldSummary, texts) {
    return [
      { role: 'system', content: '【任务：摘要】你是剧情摘要助手。把互动小说的旧剧情合并进结构化"前情提要"。固定使用以下五个板块（Markdown 小标题），总长不超过1000字，条目式书写，宁短勿虚：\n'
        + '## 时间线\n（按顺序列出关键事件：日期/时段+一句话事件，含已过去的时间跳跃）\n'
        + '## 人物关系\n（每个NPC一条：与玩家关系现状+最近互动要点，写具体事件不写抽象标签）\n'
        + '## 未解事件与伏笔\n（承诺/约定/未完成的事/埋下的线索，逐条列出）\n'
        + '## 知识边界\n（关键信息每个角色各自知道什么、不知道什么——特别是"只有玩家知道"和"某个NPC不知道另一NPC的某事"，逐条写清）\n'
        + '## 说话习惯与重要设定\n（各NPC的说话方式要点、重要物品/地点/习惯）\n'
        + '不要文学化心理描写，不要评论。已有内容与新剧情冲突时，以新剧情为准并保留双方要点。' },
      { role: 'user', content: `已有前情提要：\n${oldSummary || '（无）'}\n\n请把以下新剧情合并进前情提要（保持五板块结构，合并后依然≤1000字）：\n${texts}` }
    ];
  }
