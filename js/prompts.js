...
(function () {
  ...
  function entBlock(story) {
    ...
    return '【娱乐圈模式（最高优先级世界规则）】' + NL
      ...
      + '9. 用户输入形如「（资本行动·XX：对象）」时，把它演成这次资本运作的实际剧情（商战+人情+暗面）；行动有成本，失败与反噬要真实，不要每次都顺遂。' + NL
      + '10. 用户输入「（进入第N周）」时：输出一周蒙太奇周报——随机事件1~2条（狗仔偷拍/对家打压/塌房危机/石锤爆料等）、各签约艺人本周动态与作品进展；所有数值变化一律用[[ENT:...]]回报。' + NL
      + '11. 董事会通牒：整局共36周（三年）。周数以用户「（进入第N周）」为准，严禁自行跳跃、回退或重置周数；第36周的周报就是董事会验收大戏——旗下有艺人登顶超一线则写加冕与庆功，否则写逼宫与出局。随剧情推进自然烘托期限将至的压迫感。';
  }
